...
public class StringUtil {
	...
	public static void main(String[] args) {
		...
		System.out.println("字符串[" + test2 + "]去掉转义字符为：" + replaceString(test2)
				+ "");
	}
	...
	public static String replaceString(String srcString) {
		...
		for (int i = 0; i < jsonString.length; i++) {
			destString = destString.replaceAll("\\" + jsonString[i], "\\\\"
					+ jsonString[i]);
	...
	public static boolean isMobile(String content) {
		...
		if (StringUtils.isNumeric(content) && content.trim().length() == 11) {
			return Boolean.TRUE;
		} else {
			return Boolean.FALSE;
		}
